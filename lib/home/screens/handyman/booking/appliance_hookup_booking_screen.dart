...
class ApplianceHookupBookingScreen extends StatefulWidget {
  final Map<String, dynamic> item;

  const ApplianceHookupBookingScreen({super.key, required this.item});

  @override
  State<ApplianceHookupBookingScreen> createState() =>
      _ApplianceHookupBookingScreenState();
}

class _ApplianceHookupBookingScreenState
    extends State<ApplianceHookupBookingScreen> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _title("Select Appliance"),
            _card(
              child: Wrap(
                spacing: 10,
                runSpacing: 10,
                children: appliances.map((appl) {
                  final isSelected = selectedAppliance == appl;
                  return ChoiceChip(
                    label: Text(appl),
                    selected: isSelected,
                    selectedColor: green,
                    ...
                    onSelected: (_) {
                      setState(() => selectedAppliance = appl);
                    },
                  );
                }).toList(),
              ),
            ...
            _card(
              child: TextField(
                ...
                decoration: const InputDecoration(
                  border: InputBorder.none,
                  hintText: "Example: old appliance removal? gas or electric?",
                ),
              ),
            ),
            const SizedBox(height: 25),
